...
<head>
  ...
  <title>Sửa Tin Tức Coffee</title>
...
<div class="content-wrapper" style="min-height: 1071.31px;    background-color: #fff">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1> Sửa  Tin Tức Coffee</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Quản Trị Website</a></li>
            <li class="breadcrumb-item active">Sửa  Tin Tức Coffee</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
    ...
    <section class="content">
     ...
     <form  method="POST" role="form" enctype="multipart/form-data">
              <div class="form-group">
                ...
                <input type="text"
                  class="form-control" value = "<?=$row['TieuDe']?>" name="txtTieuDe" id="TenLoaiCoffee"  placeholder="Nhập Tiêu Đề Tin Tức">
              </div>
              <div class="form-group">
                <label for="">Hình Ảnh Tin</label>
                <input type="file" class="form-control-file"  <?=$row['HinhAnhTinTuc']?> name="txtHinhAnhTin" id="" placeholder="" aria-describedby="fileHelpId">
              </div>
              <div class="form-group">
                <label for="">Nội Dung Tin Tức</label>
                <textarea class="form-control" name="txtNoiDung" id="ND" rows="6"><?=$row['NoiDung']?></textarea>
              </div>
			<script> var editor = CKEDITOR.replace('ND'); </script>
             <input type="submit" name="submit" class="btn btn-success" value="Sửa Tin Tức">
             </form>
            </div>
      </div><!-- /.container-fluid -->
      <?php 
        ...
        if(isset($_REQUEST["submit"])){
          ...
          $sql = "UPDATE tintuc SET TieuDe = '$TieuDe', HinhAnhTinTuc ='$Anh', NoiDung = '$NoiDung'  WHERE iDTinTuc = $id";
          if ($conn->query($sql) === TRUE) {
            echo'
              <div class="alert alert-success">
                <strong>Success!</strong> Sửa Tin Tức Thành Công!
              </div></a>';
          } else {
